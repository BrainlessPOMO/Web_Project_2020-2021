    ...
    cCase
      ...
      .exec((error, docs) => {
        ...
        if (given_date > date) {
          ...
          if (
            docs[0].date.getFullYear() === given_date.getFullYear() &&
            docs[0].date.getMonth() === given_date.getMonth() &&
            Math.abs(docs[0].date.getDate() - given_date.getDate()) < 14
          ) {
            const alert = [
              {
                msg: "There has to be at lease 14 days gap between covid registers",
              },
            ];
            res.render("login/user", {
              page: "user",
            ...
            const alert = [
              { msg: "Your covid case has been registered! Be carefull!" },
            ];
    ...
    User.findOne({ _id: req.params.id }).then((user) => {
      ...
      if (!errors.isEmpty()) {
        ...
        if (username == "" && password == "") {
          ...
          //they want to change just the password
          updateUserPassword(req.params.id, password);
          res.redirect("/login");
        } else if (password == "") {
          //they want to change just the username
          updateUserUsername(req.params.id, username);
          res.redirect("/login");
        } else {
          //they want to change username & password
          updateUserPassword(req.params.id, password);
          updateUserUsername(req.params.id, username);
          res.redirect("/login");
        }
...
router.post("/admin/:id/upload", async (req, res) => {
  User.findOne({ _id: req.params.id }).then((user) => {
    try {
      ...
      const alert = [
        { msg: "The file you are trying to upload is not a JSON file" },
      ];
      res.render("login/admin", {
        page: "admin",
        user,
    ...
    try {
      ...
      dataToUpdate.forEach((data) => {
        Place.findOneAndUpdate(
          ...
          const alert = [
            {
              msg: "There was an error with your update, please check your file!",
            },
          ];
          res.render("login/admin", {
            page: "admin",
...
router.post("/admin/:id/delete", async (req, res) => {
  User.findOne({ _id: req.params.id }).then((user) => {
    Place.deleteMany({})
      .then((user) => {
        const alert = [
          { msg: "All data has been deleted. I hope you are happy :(" },
        ];
        res.render("login/admin", {
          page: "admin",
          user,
      ...
      .catch(function (e) {
        const alert = [
          {
            msg: "Data could not be deleted. I hope you did not want to destroy the website!!",
          },
        ];
        res.render("login/admin", {
          page: "admin",
